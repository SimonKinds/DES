...
void permutate(const uint8_t* input_array, const unsigned int input_bit_count,
               uint8_t* output_array, const unsigned int output_bit_cunt,
  ...
  for (unsigned int i = 0; i < output_bit_cunt; ++i) {
    ...
    uint8_t original_value =
        (input_array[original_index] & (1 << original_bit_pos)) >>
        original_bit_pos;
 ...
 * creating conflicts.
 */
__device__ void permutate_gpu(const uint8_t* input_array,
...
__device__ uint64_t initial_permutation(const uint64_t* message) {
  uint64_t permutated = 0;
  permutate_gpu((uint8_t*)message, BLOCK_SIZE_BITS, (uint8_t*)&permutated,
                BLOCK_SIZE_BITS, IP_PERMUTATION_ARRAY);
  return permutated;
}

__device__ uint64_t inverse_initial_permutation(const uint64_t* data) {
  uint64_t permutated = 0;
  permutate_gpu((uint8_t*)data, BLOCK_SIZE_BITS, (uint8_t*)&permutated,
                BLOCK_SIZE_BITS, INVERSE_IP_PERMUTATION_ARRAY);
  return permutated;
}

uint64_t key_permutation_first(const uint64_t* key) {
  uint64_t permutated = 0;
  permutate((uint8_t*)key, BLOCK_SIZE_BITS, (uint8_t*)&permutated,
            KEY_SIZE_FIRST_PERMUTATION, PC_1);
  return permutated;
}

uint64_t key_permutation_second(const uint64_t* key) {
  uint64_t permutated = 0;
  permutate((uint8_t*)key, KEY_SIZE_FIRST_PERMUTATION, (uint8_t*)&permutated,
            KEY_SIZE_SECOND_PERMUTATION, PC_2);
  return permutated;
}

__device__ uint64_t expansion_permutation(const uint32_t* r) {
  uint64_t permutated = 0;
  permutate_gpu((uint8_t*)r, BLOCK_SIZE_BITS / 2, (uint8_t*)&permutated,
                KEY_SIZE_SECOND_PERMUTATION, E);
  return permutated;
}

__device__ uint32_t p_permutation(const uint32_t* data) {
  uint32_t permutated = 0;
  permutate_gpu((uint8_t*)data, BLOCK_SIZE_BITS / 2, (uint8_t*)&permutated,
                BLOCK_SIZE_BITS / 2, P);
  return permutated;
}
...
uint64_t* generate_subkeys(const uint64_t key) {
  ...
  const uint32_t d0 = (permutated_key << KEY_SIZE_FIRST_PERMUTATION / 2) >>
                      KEY_SIZE_FIRST_PERMUTATION / 2;
...
uint32_t getC0(const uint64_t* permutated_key) {
  ...
  for (unsigned int i = 1; i < KEY_SIZE_FIRST_PERMUTATION / 8 - start_index;
       ++i) {
...
__device__ uint32_t feistal(const uint32_t* r, const uint64_t* key) {
...
//  S8 is LSB, S1 is MSB
// data is 48 bits
__device__ uint32_t s_box_transformation(const uint64_t* data) {
  uint32_t val = 0;
  // use 6 LSB
...
__device__ uint8_t s_value(const uint8_t b, const unsigned int* s_box) {
  // first and last
...
__device__ uint32_t calculate_r(const uint32_t prev_l, const uint32_t prev_r,
                                const uint64_t* key) {
  return prev_l ^ feistal(&prev_r, key);
}

__global__ void des(const uint64_t* message, const uint64_t* subkeys,
                    const unsigned int block_count, uint64_t* output_block) {
  // use grid stride looping to make sure to work on all objects
  for (unsigned int i = blockIdx.x * blockDim.x + threadIdx.x; i < block_count;
    ...
    for (unsigned int j = 0; j < AMOUNT_OF_KEYS; ++j) {
      uint32_t prev_l = l;
      l = r;
      r = calculate_r(prev_l, r, &subkeys[j]);
    }
    ...
    output_block[i] = inverse_initial_permutation(&concat);
  }
}

uint64_t* encode(const uint64_t* message, const unsigned int size,
                 const uint64_t key) {
  ...
  cudaMemcpy(subkeys_gpu, subkeys, 16 * sizeof(uint64_t),
  ...
  const int cuda_blocks =
      size / (CUDA_THREAD_COUNT_PER_BLOCK * sizeof(uint64_t));
  ...
  cudaMemcpy(encoded_message, encoded_message_gpu, size,
...
uint64_t* decode(const uint64_t* encoded, const unsigned int size,
                 const uint64_t key) {
  uint64_t* encoded_message_gpu;
  cudaMalloc(&encoded_message_gpu, size);
  ...
  cudaMemcpy(reversed_subkeys_gpu, reversed_subkeys, 16 * sizeof(uint64_t),
             cudaMemcpyHostToDevice);
  ...
  cudaMemcpy(decoded_message, decoded_message_gpu, size,
             cudaMemcpyDeviceToHost);
...
uint64_t pkcs5_padding(const uint64_t* block,
                       unsigned int amount_of_bytes_to_pad) {
...
unsigned int count_padding_bytes(const uint64_t* blocks,
                                 const unsigned int byte_count) {
  const uint8_t* bytes_in_last_block =
      &((const uint8_t*)blocks)[byte_count - 8];
  const uint8_t prev_value = bytes_in_last_block[0];
  const unsigned int byte_count_in_block = BLOCK_SIZE_BYTES;
  for (unsigned int i = 1; i < byte_count_in_block; ++i) {
    ...
    if (bytes_in_last_block[i] != prev_value) {
      // but if it's not equal to the amount of padding added, then was not
      // actually padding
      if (prev_value == i) {
        return i;
      }
...
void print_key_error_msg() { printf("Key has to be 8 characters long\n"); }

void write_to_file(const char* file_name, const uint8_t* data,
  ...
  const size_t elements_written =
      fwrite(data, sizeof(uint8_t), file_size, output_file);
...
int main(int argc, char** argv) {
  ...
  uint64_t* file_buffer =
      (uint64_t*)malloc(elements_to_read * sizeof(uint64_t));
